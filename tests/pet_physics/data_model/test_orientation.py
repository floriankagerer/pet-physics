...
def test_orientation_values() -> None:
    """Tests the values of the orientations."""
    assert Orientation.LWH.value == 0
    assert Orientation.WLH.value == 1


def test_orientation_from_value() -> None:
    """Tests whether an instance is correctly created from value."""
...
@pytest.mark.parametrize(
    ...
    [
        (Orientation.LWH, (300, 200, 100)),
        (Orientation.WLH, (200, 300, 100)),
    ],
)
def test_orientation_get_item_size_correctness(
    orientation: Orientation, expected_size: Size3d, sample_item: Item
) -> None:
    """Tests whether the dimensions are correctly swapped."""
